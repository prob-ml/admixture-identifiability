...
def fit_contribution_pmf(observation_pmf,contrib_shape,
                            ...
                            initial_guess = None, p0_guess=None):
    ...
    if true_contrib_pmf is not None:
        realshape = true_contrib_pmf.reshape(-1)[1:]
        realshape = realshape / realshape.sum()
    ...
    for i in tqdm.trange(n_iter):
        ...
        proposed_contribution_pmf = parameters_to_contribution_pmf(params, contrib_shape, contrib_pattern)
        if true_contrib_pmf is not None:
            propshape = proposed_contribution_pmf.reshape(-1)[1:]
            propshape = propshape / propshape.sum()

            # TV distance between propshape and realshape
            tvdist = 0.5 * torch.sum(torch.abs(propshape - realshape))
            secret_losses.append(tvdist.item())

        # record p0 guess
        p0_guesses.append(params[0].item())
    ...
    return params, np.array(losses), np.array(secret_losses), np.array(p0_guesses)
...
class SimulationResult:
    ...
    n_iter: int
    n: int
    p0_guesses: np.ndarray
    observation_pmf: torch.Tensor = None
    corrupted_observation_pmf: torch.Tensor = None

    def to(self, device: torch.device):
        self.params = self.params.to(device)
        if self.observation_pmf is not None:
            self.observation_pmf = self.observation_pmf.to(device)
        if self.corrupted_observation_pmf is not None:
    ...
    def strip_giant_pmfs(self):
        self.observation_pmf = None
        self.corrupted_observation_pmf = None
        return self
...
def n_sources_pmf(n,p0):
    ...
    # Binom(n, self.p0)
    return sp.stats.binom.pmf(np.r_[0:n+1],n,1-p0)


def run_simulation(n, contrib_shape, device, p0, n_samples,
                   rng, n_iter, p0_guess, log_every=None) -> SimulationResult:
    # setup problem
    ...
    initial_guess_v[0] = p0_guess
    initial_guess_v[1:] = (1-initial_guess_v[0])*initial_guess_v[1:]/initial_guess_v[1:].sum()

    # run EM
    params, losses, secret_losses, p0_guesses = fit_contribution_pmf(
