#!/usr/bin/env python3
"""Full pipeline (Phase I + II) in the *easy* oracle setting (null_prob=0.95).

This re-runs the same Phase I + Phase II loop as ``run_modal.py`` but with
``null_prob_true = 0.95`` to match the oracle experiment.  Extra diagnostics
are added at the end of Phase I (before Phase II begins) and during Phase II
(learning-rate logging, more-frequent early checkpoints) so we can pinpoint
where the bootstrap loop diverges from the oracle.

Usage:
    modal run gaubump_scripts/run_modal_easy.py
"""
...
app = modal.App("gaubump-easy-pipeline")
...
def run_training():
    """Run full pipeline at null_prob=0.95 with enhanced diagnostics."""
    import io
    import subprocess
    import threading
    ...
    # ================================================================
    # Training configuration — easy case matching oracle null_prob
    # ================================================================
    L = 3
    T = 20
    ...
    # ================================================================
    # Phase I — warm-up on pihat (null_prob=0.5)
    # ================================================================
    ...
    # ================================================================
    # Post-Phase-I diagnostic — how good is the warm-start?
    # ================================================================
    ...
    log(f"  frac(|rho|<0.1) = {frac_p1:.3f}  "
        f"(target={null_prob_true}, pihat={null_prob_init})")
    ...
    # Log more frequently early on to track convergence
    # First 1000 steps: every 100; then every 7500 like before
    ...
    ax.set_title(
        ...
        f"(trained on pihat null_prob={null_prob_init})"
    )
    ax.axvline(0, color="red", lw=0.8, ls="--", label="ρ=0 (null)")
    ax.legend(fontsize=8)
    ax.set_xlim(-1, 3.5)
    ...
    fig.suptitle(
        f"Easy pipeline  L={L}, T={T}, seed={seed}, "
    ...
    plt.close(fig)
    results["easy_posterior_vs_truth.png"] = buf1.getvalue()
    ...
    fig.suptitle("Marginal ρ distributions (easy pipeline)", fontsize=11)
    fig.tight_layout()
    buf2 = io.BytesIO()
    fig.savefig(buf2, format="png", dpi=150)
    plt.close(fig)
    results["easy_rho_marginal.png"] = buf2.getvalue()

    # ---- Plot 3: True vs inferred latent U ----
    ...
    fig.suptitle("True latent vs inferred latent (easy pipeline)",
                 fontsize=11)
    fig.tight_layout()
    buf3 = io.BytesIO()
    fig.savefig(buf3, format="png", dpi=150)
    plt.close(fig)
    results["easy_true_vs_inferred_U.png"] = buf3.getvalue()

    # ---- Plot 4: Survival function comparison ----
    ...
    fig.tight_layout()
    buf4 = io.BytesIO()
    fig.savefig(buf4, format="png", dpi=150)
    plt.close(fig)
    results["easy_rho_survival.png"] = buf4.getvalue()

    # ---- Training log ----
    training_log = "\n".join(log_lines) + "\n"
    results["easy_training_log.txt"] = training_log.encode("utf-8")
    ...
    results["easy_gpu_utilization.txt"] = gpu_utilization_log.encode("utf-8")
